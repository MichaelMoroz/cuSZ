...
int CPU_c_lorenzo_nd_with_outlier<T, UseZigZag, Eq>::kernel(
    ...
    uint16_t const radius, f4* time_elapsed)
{
  ...
  auto leap3 = psz_len{1, data_len3.x, data_len3.x * data_len3.y};
...
template <typename T, bool UseZigZag, typename Eq>
int CPU_x_lorenzo_nd<T, UseZigZag, Eq>::kernel(
    Eq* in_eq, T* in_outlier, T* out_data, psz_len const data_len3, f8 const eb,
    uint16_t const radius, f4* time_elapsed)
{
  auto d = psz_ndim(data_len3);
  auto ebx2 = eb * 2, ebx2_r = 1 / (eb * 2);  // error bound
...
template struct psz::module::CPU_c_lorenzo_nd_with_outlier<f4, true, u1>;
template struct psz::module::CPU_c_lorenzo_nd_with_outlier<f4, true, u2>;
template struct psz::module::CPU_c_lorenzo_nd_with_outlier<f4, false, u1>;
template struct psz::module::CPU_c_lorenzo_nd_with_outlier<f4, false, u2>;

template struct psz::module::CPU_x_lorenzo_nd<f4, true, u1>;
template struct psz::module::CPU_x_lorenzo_nd<f4, true, u2>;
template struct psz::module::CPU_x_lorenzo_nd<f4, false, u1>;
template struct psz::module::CPU_x_lorenzo_nd<f4, false, u2>;
