...
#define SETUP_ND_GPU_CUDA                                              \
  ...
  auto tid3 = [&]() { return t().x + t().y * TWy() + t().z * TWz(); }; \
                                                                       \
  /* global data id, TileDim is defined in function template */        \
  auto gx = [&]() { return t().x + b().x * TileDim; };                 \
  auto gy = [&]() { return t().y + b().y * TileDim; };                 \
  auto gz = [&]() { return t().z + b().z * TileDim; };                 \
  auto gid1 = [&]() { return gx(); };                                  \
  auto gid2 = [&]() { return gx() + gy() * data_leap3.y; };            \
  auto gid3 = [&]() {                                                  \
    return gx() + gy() * data_leap3.y + gz() * data_leap3.z;           \
  };                                                                   \
                                                                       \
